...
var Datastore = require('nedb'),
	...
	async = require('async')
;
var userDbFullPath,filedirs;

/**
 * 更新配置文件后回调更新方法，用来重新加载数据库信息
 * 解释：由于需要用到config的rootPath参数，而该参数是在用户选择完要打开的笔记项目时，会更新，所以需要用到该回调方法
 */
function updateConfigReloadDbs(){
	...
	filedirs = new Datastore({filename: userDbFullPath + '/filedirs.db', autoload: true});
		// gitResp = new Datastore({filename: userDbFullPath + '/gitResp.db', autoload: true})
}
...
module.exports = filedirCollection;
