...
var ROOT_PATH = path.resolve(config.get("rootPath"), "." + utils.localRepoPath);
...
angular.module("cnoteApp").service("cnoteAppService", ["$q", function ($q) {
    ...
    function createFile(parent) {
        ...
        // console.log("dirPath:" + path.resolve(ROOT_PATH, "." + dirPath));
        if (!utils.mkdirsSync(path.resolve(ROOT_PATH, '.' + dirPath))) {
            ...
            if (result instanceof Error) {
                ...
                try {
                    //console.log("path.resolve(ROOT_PATH, \".\"+file.path):", path.resolve(ROOT_PATH, "." + file.path));
                    fs.writeFileSync(path.resolve(ROOT_PATH, '.' + file.path), result, {encoding: 'utf8'});
        ...
        filedirsDao.find(query, function (err, files) {
            if (err) {
                ...
                if (!files || files.length <= 0) {
                    ...
                    var file = files[0];
                    var result = Cson.parseCSONFile(path.resolve(ROOT_PATH, "." + file.path));
                    if (result instanceof Error) {
                        deferred.reject(result);
                    } else {
    ...
    function updateFileToDB(file) {
        var deferred = $q.defer();
        if (!fs.existsSync(path.resolve(ROOT_PATH, "." + file.path))) {
        ...
        if (!fs.existsSync(path.resolve(ROOT_PATH, "." + file.path))) {
            ...
            saveFileToDB(file, function (err, result) {
                if (err) {
                    ...
                    var err = saveFileToDiskSync(file.title, file.content, path.resolve(ROOT_PATH, "." + file.path));
                    if (!!err) {
                        deferred.reject(err);
                    } else {
        ...
        if (!fs.existsSync(path.resolve(ROOT_PATH, "." + file.path))) {
            ...
            filedirsDao.foreverDeleteFile(file._id.toString(), function (err) {
                if (err) {
                    ...
                    try {
                        var result = fs.unlinkSync(path.resolve(ROOT_PATH, "." + file.path));
                        if (result) {
                            return deferred.reject(err);
                        } else {
